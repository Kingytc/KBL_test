...
String app_category=Helper.correctNull((String)hshValues.get("apprenewflag")); //pending
...
var varSendToCBSFlag = "<%=Helper.correctNull((String) hshValues.get("strSendToCBSFlag"))%>"; /* pending */

/* Shiva dev */
var varstrrevalidcomments="<%=Helper.correctNull((String)hshValues.get("strrevalidcomments"))%>";
var varstrFacsno="<%=Helper.correctNull((String)hshValues.get("strFacsno"))%>";
...
{	
	...
	//shiva Check for later...
	if(varSessionModuleType=="DIGI"||strSessionModuleType=="CORP") 
	{
	   if(varstrFacSno!="")
		{
...
function enableDepartment(val2){
	...
	/* SEND TO SUBMIT FOR THE VALIDATION */
function funLoad()
{   
	if((varSessionModuleType=="DIGI") && !(varappcategory=="S"||varappcategory=="P") && varSendToCBSFlag!="Y")
	{	
		if (strPrdType=="pH" || strPrdType=="pM"||  strPrdType=="pA")
		{	
		...
		{	
			for(int i=0;i<arrRevalidation.size();i++)
			{
				...
				System.out.println("strProcessDate"+strProcessDate);
				System.out.println("getCurrentDateTime()"+Helper.getCurrentDateTime());
				System.out.println("lngDayDiff1 "+lngDayDiff1);
				System.out.println("lngDayDiff "+lngDayDiff);
				System.out.println("lngDayDiff1 in month"+Helper.daysToMonths(lngDayDiff1));
				%>
		...
		<%if(arrRevalidation!=null && arrRevalidation.size()>0)
		{
			for(int i=0;i<arrRevalidation.size();i++)
			{
				...
				if(strRevalidationDate.equalsIgnoreCase(""))
					strRevalidationDate=Helper.getCurrentDateTime();
				lngDayDiff=Helper.dateDiff_new(strProcessDate,Helper.getCurrentDateTime());
				lngDayDiff1=Helper.dateDiff_new(strRevalidationDate,Helper.getCurrentDateTime());%>
				<%if(lngDayDiff>Integer.parseInt(Helper.correctInt((String)arrCol.get(0)))
						&&(Helper.correctNull((String)arrCol.get(6)).equalsIgnoreCase("Y") && 
								(Helper.correctNull((String)arrCol.get(7)).equalsIgnoreCase("pa")||Helper.correctNull((String)arrCol.get(7)).equalsIgnoreCase("pr")))){%>
						if(document.forms[0].appholder.value==strUserId && (document.forms[0].appstatus.value=="Processed/Approved"))
						{
							<%if(Helper.correctNull((String)arrCol.get(7)).equalsIgnoreCase("pa")){
							//if(lngDayDiff1<=Integer.parseInt(Helper.correctInt((String)arrCol.get(1))))
								if(lngDayDiff1<=Integer.parseInt(Helper.correctInt((String)arrCol.get(2))))
							{%>
								document.forms[0].cmdAdditionalparameter.disabled=false;
								document.forms[0].cmdRevalidate.disabled=false;
								if(varstrrevalidcomments=="Y")
...
function doValidateCheck()
{
	debugger
	var facsno="1";
	if((varSessionModuleType=="AGR"||varSessionModuleType=="CORP") && document.forms[0].sel_facility.value=="s")
	{
...
<body onLoad="callOnload();permissionundo();funLoad();">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" class="outertable">
	<tr align="center" >
  	...
  	<input id="PropRevalid" type="button" name="cmdRevalidate" class="buttonStyle"  value="Revalidate" onClick="doValidateCheck()" disabled="disabled">&nbsp;
