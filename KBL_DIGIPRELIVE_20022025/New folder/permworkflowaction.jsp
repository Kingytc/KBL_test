...
var varprdbankscheme="<%=Helper.correctNull((String)hshValues.get("prd_bankscheme"))%>";
...
function checkSplDotCQ(){

	const str = document.forms[0].txt_sanctioncomments.value;
    const removed = str.replaceAll('"', '');
    const removed1 = removed.replaceAll('\n', '');
    document.forms[0].txt_sanctioncomments.value = removed1;
...
function showSanctionDetails()
{
	...
	{ 
		...
		alert("Kindly process account opening request in LAPS - KBL PM Vishwakarma application");
		return;
	}
}
...
function doPermit()
{
	...
	if(varSessionModuleType!="DIGI")
	{
	alert("Your application is Approved and transferred to Branch Head login successfully");
	}
}

function goSubmit()	//Approve Button function.
{
	checkSplDotCQ();
	if(Varseccroptype == 'Y')
	{
...
{	
	...
	{	
		if (strPrdType=="pH" || strPrdType=="pM"||  (strPrdType=="pA" && strprd_catid=="59"))
		{	
...
<tr align="center" >
 <td>
  <input type="button" class="buttonOthers" name="cmdpermit" value="Approve" onclick="goSubmit()">&nbsp;
